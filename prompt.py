...
constraints =[
    ...
    "你无法与物理对象交互,如果对于完成任务或目标是绝对必要的, 则必须要求用户为你完成,如果用户拒绝,并没有其他方法实现目标,则直接终止,避免浪费时间和精力."
]
...
best_practices = [
    "不断地回顾和分析你的行文,确保发挥出你最大的能力",
    "不断地进行建设性的自我批评",
    "反思过去的决策和策略, 完善你的方案",
    "每个动作执行都有代价, 所以要聪明高效,目的是用最少的步骤完成任务",
    "利用你的信息收集能力来寻求你不知道的信息",
]
...
agent_cratch:{agent_cratch}

你应该只以json格式响应, 相应格式如下:
{response_format_prompt}
你得必须遵守我给你的这个格式,确保响应结果可以由python json.loads()成功加载
"""
...
def gen_prompt(query, agent_cratch):
...
user_prompt = "根据给定的目标和迄今为止取得的进展, 确定下一个要执行的action, 并使用前面制定的JSON模式进行响应"
